...
if ( $theDropbox = new NSSDropbox($NSSDROPBOX_PREFS) ) {
  ...
  if ( $thePickup = new NSSDropoff($theDropbox) ) {
    ...
    // The TRUE tells it to reset the expiry timer.
    $success = $thePickup->resendDropoff(TRUE);
    $smarty->assign('success', $success);
    $smarty->assign('autoHome', TRUE);
    
    if ( ! $success ) {
      NSSError(sprintf(gettext("Unable to re-send the drop-off %s."), $claimID).' '.$SYSADMIN, gettext("Unable to Re-send Drop-off"));
    }
  }
}
